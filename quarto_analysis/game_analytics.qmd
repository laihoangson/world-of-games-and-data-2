...
## 3. Data Splitting & Cross-Validation Strategy

Given the dataset's size (300 samples), artificially inflating the data through simple bootstrapping risks massive overfitting, where models memorize duplicated records rather than learning underlying patterns. To build robust models, I rely on K-Fold Cross-Validation during training and a strict holdout set for final evaluation.

I split the data chronologically to mimic a real-world scenario (predicting future sessions based on past ones), reserving the last 50 sessions as our unbiased test set. The remaining 250 sessions form our base training set.
...
# Define Cross-Validation strategy for caret
...
features_to_cluster <- train_base %>% 
    select(score, game_duration, coins_collected, bullets_fired, ufos_shot, 
           pipes_passed, aggressiveness, efficiency, accuracy, risk_taking)
...
train_base$cluster <- as.factor(kmeans_model$cluster)

# Visualize clusters
fviz_cluster(kmeans_model, data = scaled_features, geom = "point", 
...
- Cluster 1 (23 samples): The "Experts". This group has a very high average score (53.2), a long game duration (37s), and high numbers for coins, UFOs shot, and bullets fired. They are highly skilled and engaged players.

- Cluster 2 (75 samples): The "Novices". This group has an extremely low average score (0.4) and a very short game duration (1.4s). These are likely new players who fail almost immediately and are at the highest risk of churning.

- Cluster 3 (152 samples): The "Average Players". This group sits between the other two, with a moderate average score (10.8) and game duration (7.6s). They represent the bulk of the player base who have passed the initial learning curve but have not yet reached expert level.

## 5. Predictive Modeling (Supervised Learning)

Leveraging the insights and the enhanced dataset, this section focuses on building predictive models using supervised learning. The objective is to tackle three distinct business problems: predicting a player's final score (Score Regression), predicting whether a player will survive longer than a 30-second threshold (Survival Prediction), and predicting the specific cause of death (Multiclass Classification). Advanced algorithms like Random Forest and XGBoost are trained on the train base data and evaluated on the holdout test set.
...
# Predict and assess
rf_preds <- predict(rf_score_model, newdata = test_holdout)
rmse_val <- RMSE(rf_preds, test_holdout$score)
r2_val <- R2(rf_preds, test_holdout$score)
...
RMSE: 1 and R-Squared: 0.9945: These are performance metrics for the Random Forest model predicting the final score. The RMSE (Root Mean Squared Error) of 1 indicates that, on average, the model's score predictions are off by only 1 points, which is very low. The R-Squared value of 0.9945 is extremely high, signifying that the model can explain 99.45% of the variability in the score, making it an exceptionally accurate predictor.
...
rf_surv_model <- train(
  survived_expert ~ bullets_fired + ufos_shot + coins_collected + aggressiveness 
  + efficiency + accuracy + pipe_pass_rate,
  data = train_base,
  method = "rf",
  trControl = fit_control
)
...
# Predictions
surv_preds <- predict(rf_surv_model, newdata = test_holdout)

# Evaluation
conf_mat <- confusionMatrix(surv_preds, test_holdout$survived_expert)
cat("Random Forest Accuracy:", round(conf_mat$overall['Accuracy'], 4), "\n")
...
xgb_train <- as.matrix(train_base[xgb_features])
xgb_test <- as.matrix(test_holdout[xgb_features])
xgb_labels_train <- as.numeric(as.character(train_base$survived_expert))
...
While this analysis provides robust and actionable insights into player behaviour, several limitations inherent to the dataset and methodology should be acknowledged:

- Sample Size Constraints: The original dataset consists of only 300 game sessions. Although the implementation of 5-Fold Cross-Validation effectively maximises data utility and prevents overfitting during model evaluation, it cannot artificially create new, distinct playstyles. The models' ability to generalise to a broader, global player base is still inherently bounded by the variance present in these initial 300 samples.

- Deterministic Target Variables: The predictive models, particularly the Score Regression model, achieved exceptionally high accuracy (R^2 > 0.99). While Cross-Validation ensures this is not due to data leakage, this near-perfect performance stems from the deterministic nature of the game's scoring engine. Because the final score is a strict mathematical combination of `coins_collected` and `ufos_shot`, the Random Forest algorithm essentially reverse-engineered the game's mathematical formula rather than inferring deep, hidden behavioural complexities.

- Class Imbalance in Multiclass Classification: The death reason prediction model is significantly constrained by class imbalance. A vast majority of game sessions end in "pipe" or "ground" collisions, leaving highly advanced failure states (like "enemy_bullet" or "ufo_collision") underrepresented. This imbalance reduces the model's reliability in accurately classifying those rarer, high-skill death events.

- Correlated Behavioral Features: Several engineered features, such as `efficiency`, `risk_taking`, and `ufo_rate`, are highly correlated. While this collinearity does not degrade the predictive power of algorithms like Random Forest or XGBoost, it does entangle feature importance scores, making it slightly more challenging to isolate the independent impact of a single behavioral metric.

- Lack of Longitudinal Data: The current analysis is strictly session-based. Without unique player IDs linking multiple sessions together, it is impossible to track longitudinal trends, such as individual player learning curves, adaptation to difficulty over time, or long-term retention rates.

Future work should aim to collect larger-scale, longitudinal data with unique player identifiers. Tracking players across multiple sessions would allow for advanced cohort analysis, deeper insights into the learning process, and the development of predictive models that forecast long-term player churn and lifetime value (LTV).
