...
1. Unsupervised Learning (Segmentation): Principal Component Analysis (PCA) and K-Means clustering are applied to the full dataset prior to any split, revealing three distinct player personas: Novices, Average Players, and Experts.
...
```{r}
class_names <- c("ceiling", "enemy_bullet", "ground", "pipe", "ufo_collision")

# Confusion matrix
conf_matrix <- table(
  Predicted = factor(pred_classes, levels = 0:4, labels = class_names),
  Actual = factor(y_test, levels = 0:4, labels = class_names)
)
...
```{r}
#| echo: false
death_type_table <- data.frame(
  Death_Type = c("Ground", "Pipe", "Enemy Bullet", "Ceiling", "UFO Collision"),
  Test_Count = c(15, 72, 13, 1, 1),
  Recall = c("86.7%", "93.1%", "15.4%", "0%", "0%"),
  Skill_Level = c("Novice - early failure", 
...
kable(death_type_table, 
      caption = "Death Reason Prediction Performance by Skill Level",
      align = "c",
      col.names = c("Death Type", "Test Count", "Recall", "Skill Level"))
```
